...
%       equation of fractional order (FDE) by the predictor-corrector PECE 
%       method of Adams-Bashforth-Moulton type described in [1].
%
...
%   set of initial conditions Y0 is a column vector when 0<ALPHA<1 and a 
%   matrix with a number of rows equal to the size of the problem (hence 
%   equal to the number of rows of the output of FDEFUN) and a number of 
%   columns that depends on ALPHA and given by m. The step-size H>0 is 
%   constant throughout the integration.   
%
%   [T,Y] = FDE12(ALPHA,FDEFUN,T0,TFINAL,Y0,H,PARAM) solves as above with a
%   possible additional set of parameters of the FDEFUN as FDEFUN(T,Y,PARAM).
%
...
%   MU = 0 : corrector is not evaluated and the solution is provided by the
%            predictor method (first order rectangular rule);
%   MU > 0 : corrector is evaluated by the selected number MU of times; the 
%            classical PECE method is obtained with MU=1 (default);
%   MU = Inf : corrector is evaluated for a certain number of times until 
%            convergence of the iterations is reached (for convergence the
...
%   solution is evaluated, i.e. N = (TFINAL-T)/H. Stability properties of
%   the method are studied in [5].
%
...
max(size(alpha))
if max(size(alpha)) > 1
    error('MATLAB:fde12:OrderNotScalar',...
        'The order ALPHA of the FDE must be a positive scalar value.');    
end
if alpha <= 0
...
% Structure for storing information about the problem 
Probl.ic = ic ;
Probl.fdefun = fdefun ;
...
y(:,1) = y0(:,1) ;
fy(:,1) = f_temp ;
[y, fy, Probl] = Triangolo(1, r-1, t, y, fy, zn_pred, zn_corr, N, PC, Probl ) ;
% Main process of computation by means of the FFT algorithm
ff = [0 2 ] ; nx0 = 0 ; ny0 = 0 ;
for q = 0 : Q
    L = 2^q ; 
    [y, fy, Probl] = DisegnaBlocchi(L, ff, r, Nr, nx0+L*r, ny0, t, y, fy, ...
...
function [y, fy, Probl] = DisegnaBlocchi(L, ff, r, Nr, nx0, ny0, t, y, fy, ...
                                  zn_pred, zn_corr, N , PC, Probl)
...
while ~stop
    ...
    [zn_pred, zn_corr, Probl] = Quadrato(nxi, nxf, nyi, nyf, fy, zn_pred, zn_corr, PC, Probl) ;
    
    [y, fy, Probl] = Triangolo(nxi, nxi+r-1, t, y, fy, zn_pred, zn_corr, N, PC, Probl) ;
...
% =========================================================================
% =========================================================================
function [zn_pred, zn_corr, Probl] = Quadrato(nxi, nxf, nyi, nyf, fy, zn_pred, zn_corr, PC, Probl)
...
if Probl.complex
zzn_pred = FastConv(vett_coef,vett_funz) ;
else
...
if PC.mu > 0
    ...
    if Probl.complex
    zzn_corr = FastConv(vett_coef,vett_funz) ;
    else
...
% =========================================================================
% =========================================================================
function [y, fy, Probl] = Triangolo(nxi, nxf, t, y, fy, zn_pred, zn_corr, N, PC, Probl)
for n = nxi : min(N,nxf)
    ...
    if PC.mu == 0
        ...
        y(:,n+1) = yn1 ;
        fy(:,n+1) = fn1 ;
        % Check whether the problem is complex valued
        if ~Probl.complex && max(abs(imag(yn1))) > 0
            Probl.complex = 1 ;
        end
    end
end
