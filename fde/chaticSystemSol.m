...
switch system_choice
    ...
    case 'Chua'
        p.p = 10; p.q = 14.87; p.r = 18; p.m0 = -1.143; p.m1 = -0.714;
        chua_diode = @(x, m0, m1) m1*x + 0.5*(m0 - m1)*(abs(x+1) - abs(x-1));
        y0 = [0.1, 0.2, -0.1];

        % CORRECTED: Use commas for a row vector output
        f = @(t, y, p) [ p.p * (y(2) - y(1) - chua_diode(y(1), p.m0, p.m1)), ...
                         p.q * (y(1) - y(2) + y(3)), ...
                        -p.r * y(2) ];
